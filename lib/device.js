	...
	addAgenda: (deviceId, agendaName, timeString) => {
		...
		return new Promise((resolve, reject) => {
			...
			.then((newAgenda) => {
				...
				else if (agendaName === 'deviceOn' || agendaName == 'deviceOff'){
					scheduler.scheduleTask(timeString, agendaName, { deviceId: deviceId })
						.catch((err) => reject(err))
						.then(() => resolve());
				}
			});
			...
			async.waterfall([
				...
				(d, next) => {
					async.map(d.DeviceAgendas, (da, callback) => {
						scheduler.cancelDeviceJob(da.agendaJobName, da.deviceId)
	...
	unpauseDevice: (id) => {
		return new Promise((resolve, reject) => {
			async.waterfall([
				...
				(d, next) => {
					async.map(d.DeviceAgendas, (da, callback) => {
						scheduler.addDeviceJob(da.agendaJobName, da.deviceId, da.timeString)
							.catch((err) => callback(err))
							.then(() => {
								da.isPaused = false;
								da.save()
