...
let scheduler = {
	...
	defineJobs: () => {
		scheduler.jobs = {
			...
			logDht22Data: (job, done) => {
				async.waterfall([
					(next) => {
						devices.getAllDht22()
							.catch((err) => next(err))
							.then((dht22s) => next(null, dht22s));
					},
					(dht22s, next) => {
						async.each(dht22s,
							(d, callback) => {
								gpio.readSensor(d.sensorId)
									...
									.then((sd) => {
										let logData = {
											deviceId: d.id,
											temperature_f: sd.temperature,
											humidity: sd.humidity
										};
										db.TemperatureLog.create(logData)
											.catch((e) => {
	...
	startTempLogJob: (cb) => {
		scheduler.agenda.jobs({ name: 'logDht22Data' }, (err, jobs) => {
			if (err)
				return cb(err);
			else if (jobs && jobs.length > 0)
				return cb();

			let job = scheduler.agenda.create('logDht22Data');
			job.repeatEvery('5 minutes');
			job.save((err) => {
				if (err){
					console.error('Unable to start temperature logger job');
					return cb(err);
				}
				return cb();
			});
	...
	agendaReady: () => {
		async.series([
			...
			(next) => scheduler.startTempLogJob(next)
		],
		(err, results) => {
			if (err)
